...
export default function Page() {
  ...
  const { data, count, totalPages, loading, error } = useLabores(busqueda, page, 50, proyecto?.nombre)
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-center gap-4">
            <BackButton />
            <h1 className="text-xl font-bold text-white">Labores</h1>
          </div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={onKey}
            placeholder="Buscar por código o labor..."
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-800/50">
                ...
                <th className="text-left py-3 px-4 text-slate-400 font-medium">Descripción (Labor)</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="py-12 text-center text-slate-500">
                    {error ? 'Tabla de labores no existe o hubo un error.' : 'No se encontraron labores'}
                  </td>
                </tr>
              ) : (
                data.map((p: any) => (
